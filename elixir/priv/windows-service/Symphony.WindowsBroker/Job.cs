...
sealed class KillOnCloseJob : IDisposable
{
    ...
    public KillOnCloseJob()
    {
        ...
        var info = new ExtendedInfo { Basic = new BasicInfo { LimitFlags = KillOnClose } };
        var size = Marshal.SizeOf<ExtendedInfo>();
        var pointer = Marshal.AllocHGlobal(size);
        try
        {
            Marshal.StructureToPtr(info, pointer, false);
            if (!SetInformationJobObject(handle, Extended, pointer, (uint)size)) throw new Win32Exception();
        }
        finally { Marshal.FreeHGlobal(pointer); }
    }

    public void Add(System.Diagnostics.Process process) => Add(process.SafeHandle);
    public void Add(SafeHandle processHandle)
    {
    ...
    [DllImport("kernel32.dll", SetLastError = true)] static extern bool AssignProcessToJobObject(SafeFileHandle job, SafeHandle process);
}
