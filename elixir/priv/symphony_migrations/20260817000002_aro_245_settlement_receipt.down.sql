...
delete from symphony_staging.effect_operations
where effect_type = 'review_settlement_receipt';
...
begin
  ...
  restored_definition := replace(
    function_definition,
    $needle$'linear_issue_create', 'github_review_thread_resolve',
    'review_settlement_receipt'$needle$,
    $replacement$'linear_issue_create', 'github_review_thread_resolve'$replacement$
  );
  if restored_definition = function_definition then
    raise exception 'begin_effect allowlist shape is incompatible with settlement receipt rollback';
  end if;
...
set contract_version = 2,
    migration_name = '20260817000000_aro_245_review_settlement_effects',
    installed_at = clock_timestamp()
where contract_name = 'effect-ledger';

update symphony_staging.contract_versions
set contract_version = 2,
    migration_name = '20260817000001_aro_245_durable_settlement_readback',
    installed_at = clock_timestamp()
