defmodule SymphonyElixir.EffectLedger do
  ...
  def execute(connection, effect_type, context, adapter, reconciler)
    ...
    with {:ok, status, resource} <- begin_effect(connection, effect_type, context) do
      continue(connection, status, resource, context, adapter, reconciler)
    end
  end
  ...
  defp continue(_connection, "succeeded", resource, _context, _adapter, _reconciler),
    do: {:ok, resource}

  defp continue(connection, status, _resource, context, adapter, reconciler)
    ...
    case reconciler.() do
      ...
      :not_found when status == "pending" ->
        perform(connection, context, adapter)
  ...
  defp continue(_connection, status, _resource, _context, _adapter, _reconciler),
    do: {:error, {:invalid_effect_status, status}}

  defp perform(connection, context, adapter) do
    case adapter.() do
      {:ok, resource} ->
        case finish_effect(connection, context, "succeeded", resource, nil) do
          :ok -> {:ok, resource}
          {:error, reason} -> {:error, {:ledger_completion_unknown, reason}}
        end

      {:error, :no_effect, reason} ->
        with :ok <- finish_effect(connection, context, "failed-no-effect", nil, inspect(reason)) do
          {:error, {:failed_no_effect, reason}}
        end

      {:error, :unknown, reason} ->
        with :ok <- finish_effect(connection, context, "unknown", nil, inspect(reason)) do
  ...
  defp begin_effect(connection, effect_type, context) do
    sql = """
    ...
    case Postgrex.query(connection, sql, params) do
      {:ok, %Postgrex.Result{rows: [[status, resource]]}} -> {:ok, status, resource}
      {:ok, result} -> {:error, {:unexpected_begin_effect_result, result.num_rows}}
      {:error, reason} -> {:error, reason}
    end
  end

  defp finish_effect(connection, context, status, resource, failure_reason) do
    sql = "select symphony_staging.finish_effect($1, $2, $3, $4::jsonb, $5)"
    params = [context.operation_id, context.request_fingerprint, status, encode_resource(resource), failure_reason]
