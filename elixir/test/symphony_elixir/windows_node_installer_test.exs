defmodule SymphonyElixir.WindowsNodeInstallerTest do
  ...
  test "installer retains one broker-only identity boundary" do
    ...
    assert installer =~ "$bundleExtractRoot = Join-Path $brokerRoot 'bundle-cache'"
    assert installer =~ "DOTNET_BUNDLE_EXTRACT_BASE_DIR=$bundleExtractRoot"
    assert installer =~ "PropertyType MultiString"
    assert installer =~ "service_bundle_environment_mismatch"
    assert installer =~ "Set-ProtectedAclRules $bundleExtractRoot"
    assert installer =~ "@{ Principal = $serviceIdentity; Rights = 'FullControl' }"
